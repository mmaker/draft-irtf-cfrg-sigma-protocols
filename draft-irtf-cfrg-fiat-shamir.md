...
We focus on the security guarantees of the non-interactive Fiat-Shamir transformation, where they provide the following guarantees (in the random oracle model) for Non-interactive Sigma Protocols:

- **Knowledge soundness**: Non-interactive Sigma Protocols preserve the knowledge soundness properties of Interactive Sigma Protocols. If the proof is valid, the prover must have knowledge of a secret witness satisfying the proof statement. This property ensures that valid proofs cannot be generated without possession of the corresponding witness.
