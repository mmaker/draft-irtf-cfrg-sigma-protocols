...
def _emit(f, lhs, payload, width=68):
    """Write `lhs + payload`, wrapping a hex payload on even-offset boundaries."""
    if _is_hex(payload):
        first = max(2, (width - len(lhs)) // 2 * 2)  # even chars that fit after lhs
        f.write(lhs + payload[:first] + "\n")
        cont = width // 2 * 2  # even chars per continuation line
        rest = payload[first:]
        for i in range(0, len(rest), cont):
            f.write(rest[i : i + cont] + "\n")
    else:
        f.write(lhs + payload + "\n")


def write_txt(path, vectors):
        ...
        for v in vectors:
            ...
            for key, value in v.items():
                if key == "Title":
                    continue
                if key == "Operations":
                    # one un-numbered line per operation; order is line order
                    for op in value:
                        if op["type"] == "absorb":
                            _emit(f, "Operation = absorb:", op["data"] or '""')
                        else:
                            _emit(f, "Operation = squeeze:", str(op["length"]))
                    continue
                if isinstance(value, list):
                    # a sequence value: one keyed line per element, in order
                    for item in value:
                        _emit(f, f"{key} = ", str(item))
                    continue
                _emit(f, f"{key} = ", str(value))
            f.write("~~~\n\n")
