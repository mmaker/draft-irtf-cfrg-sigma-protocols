...
The public functions are obtained relying on an internal structure containing the definition of a Sigma Protocol.
    ...
    class SigmaProtocol:
       ...
       def prover_commit(self, witness, seed) -> (commitment, prover_state)
       ...
       # optional
       def simulate_response(self, seed) -> response
       # optional
...
- `prover_commit(self, witness: Witness, seed) -> (commitment, prover_state)`, denoting the **commitment phase**, that is, the computation of the first message sent by the prover in a Sigma Protocol. This method outputs a new commitment together with its associated prover state, depending on the witness known to the prover, the statement to be proven, and a 32-byte seed. The seed MUST be generated by a cryptographically secure pseudorandom number generator (CSPRNG) and MUST be unique for each proof invocation. The seed is expanded using the XOF interface as specified in {{Section 4 of !FS=I-D.draft-irtf-cfrg-fiat-shamir-00}} to generate the commitment nonces. This step generally requires access to a high-quality entropy source to generate the seed. Leakage of even just a few bits of the commitment could allow for the complete recovery of the witness. The commitment is meant to be shared, while `prover_state` must be kept secret.
...
- `simulate_response(self, seed) -> response`, denoting the first stage of the simulator. The `seed` parameter is a 32-byte seed generated by a CSPRNG, expanded using the XOF interface as specified in {{Section 4 of !FS=I-D.draft-irtf-cfrg-fiat-shamir-00}}.
...
- `random(seed: bytes, dst: bytes)`: returns an element sampled uniformly at random from the group. The `seed` is a 32-byte value generated by a CSPRNG, and `dst` is a domain separation tag (UTF-8 string) as specified in {{Section 4 of !FS=I-D.draft-irtf-cfrg-fiat-shamir-00}}.
...
- `random(seed: bytes, dst: bytes)`: returns an element sampled uniformly at random from the scalar field in the range `[1, order - 1]`. The `seed` is a 32-byte value generated by a CSPRNG, and `dst` is a domain separation tag (UTF-8 string) as specified in {{Section 4 of !FS=I-D.draft-irtf-cfrg-fiat-shamir-00}}.
...
#### Prover commitment
    ...
    prover_commit(self, witness, seed)

    Inputs:

    - witness, an array of scalars
    - seed, a 32-byte seed generated by a CSPRNG
    ...
    1. dst = "SigmaProof-nonce"
    2. nonces = Xof.expand_into_vec(seed, dst, self.instance.Domain.order, self.instance.linear_map.num_scalars)
    3. prover_state = self.ProverState(witness, nonces)
    4. commitment = self.instance.linear_map(nonces)
    5. return (prover_state, commitment)
